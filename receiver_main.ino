        ...
        while (client.connected()) {
            if (client.available()) {
                ...
                if (receivedCommand == 1) {  // Forward
                    ...
                    stopWheels();
                } else if (receivedCommand == 3) {  // Right
                    Serial.println("Turning Right");
                    turnRight();
                } else if (receivedCommand == 4) {  // Left
...
void turnRight() {
    digitalWrite(M1, LOW);
    analogWrite(E1, 128);  // Set motor speed if using PWM
    digitalWrite(M2, HIGH);
    analogWrite(E2, 255);  // Set motor speed if using PWM
    delay(100);  // Small delay to ensure right turn
}
...
void turnLeft() {
    ...
    analogWrite(E1, 255);  // Set motor speed if using PWM
    digitalWrite(M2, HIGH);
    analogWrite(E2, 128);  // Set motor speed if using PWM
    delay(100);  // Small delay to ensure left turn
}
